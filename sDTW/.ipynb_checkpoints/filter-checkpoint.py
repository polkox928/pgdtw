...
_ = Parallel(n_jobs=num_cores, verbose = 100)(delayed(processBatch)(ID) for ID in IDs)
...
train_columns = ["refID", "t_query", "t_ref", "T_ref", "DTW_dist"]
target = ["T_query"]
...
def predictBatch(queryID):
    ...
    X = pd.get_dummies(train_dataset.loc[:, train_columns], columns = ["refID"]).values
...
# =============================================================================
    X_test = pd.get_dummies(test_dataset.loc[:, train_columns], columns = ["refID"]).values
...
_ = Parallel(n_jobs=num_cores, verbose = 100)(delayed(predictBatch)(queryID) for queryID in IDs)
