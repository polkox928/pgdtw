...
for (filename in list.files(path = "withRefDummyShape/prediction")){
...
for (batchID in batches){
    ...
    df = read.csv(paste("withRefDummyShape/prediction/pred", batchID, ".csv", sep = ""))
    ...
    png(paste("withRefDummyShape/prediction/plots/",batchID, ".png", sep = ""), width = 10, height = 8, units = "in", res = 300)
