...
def generate_dataset_xy(t_ref, t, ongoing_id, D):
    data_set = list()
...
for idx in tqdm(range(11, len(ids)), desc='Main Loop'):
    ongoing_id = ids[idx]
    if '%s.csv'%ongoing_id in os.listdir('cv_online/'): continue
    hist_data = deepcopy({_id : copy(all_data[_id]) for _id in ids[:idx-1]})
    data = lib.assign_ref(hist_data)
    data[ongoing_id] = deepcopy(all_data[ongoing_id])
    D = deepcopy(lib.Dtw(data))
    if len(D.data['queries']) < 5:
        continue

    for _id in tqdm(D.data['queriesID'], desc='Open-ended DTW', leave=False):
        D.call_dtw(_id, step_pattern=step_pattern, n_jobs=1, open_ended=True, all_sub_seq=True)

    estimator = Estimator(dtw_obj=D, regressor=LinearRegression(), learning_rate=0.1, max_depth=10)

    for (i, j, d) in tqdm(D.data_open_ended['warp_dist'][ongoing_id], desc='%d/%d'%(idx, len(ids)+1)):

        data_x, data_y = deepcopy(generate_dataset_xy(i, j, ongoing_id, D))
